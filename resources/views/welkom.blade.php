...
@section('main')
        ...
        <div class="flex flex-col flex-grow rounded-md p-1 justify-evenly">
            <div>
                ...
                <form action="{{ route('tafel_sessie.store') }}" method="post" class="space-y-4">
                    @csrf
                    <input type="text" inputmode="numeric" pattern="[0-9]*" name="tafel_nummer" id="tafel_nummer" class="w-full bg-stone-200 border border-teal-700 rounded py-2 px-3 focus:outline-teal-500" placeholder="Voer tafelnummer in">
    ...
    <section class="flex flex-col min-h-[60vh] max-w-lg mx-auto text-center justify-center">
        <form action="{{ route('codes.login') }}" method="post">
            ...
            <input type="text" inputmode="numeric" pattern="[0-9]*" name="code" id="code" class=" my-1 bg-stone-200 border border-teal-700 border-2 rounded"><br>
